...
public class TC002_GenericManagenent_Diagosis extends BaseClass{
  
  @Test(priority = -1)

	public void Diagosis() throws InterruptedException, IOException {
	  readConfig();
		String path = prop.getProperty("Coppied");
		File file = new File(path);
		FileInputStream fis = new FileInputStream(file);
		XSSFWorkbook wb = new XSSFWorkbook(fis);
		XSSFSheet sheet = wb.getSheet("CreateDiagnosis");
...
//		System.out.println(sheet.getLastRowNum());
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			if (row != null) {
				...
				Thread.sleep(100);
				driver.findElement(By.xpath(prop.getProperty("diagnosisBtn"))).click();
				// press on create Diagnosis
