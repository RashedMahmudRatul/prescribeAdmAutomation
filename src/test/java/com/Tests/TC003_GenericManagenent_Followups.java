...
public class TC003_GenericManagenent_Followups extends BaseClass {

	@Test(priority = -1)

	public void Followups() throws InterruptedException, IOException {
		readConfig();
		String path = prop.getProperty("Coppied");
		File file = new File(path);
		FileInputStream fis = new FileInputStream(file);
		XSSFWorkbook wb = new XSSFWorkbook(fis);
		XSSFSheet sheet = wb.getSheet("CreateFollowup");
...
//		System.out.println(sheet.getLastRowNum());
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			// go to Followups type page
			driver.findElement(By.xpath(prop.getProperty("gmBtn"))).click();
			Thread.sleep(100);
			driver.findElement(By.xpath(prop.getProperty("followupsBtn"))).click();
			// press on create Followups type
