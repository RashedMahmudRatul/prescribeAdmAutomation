...
public class TC006_GenericManagenent_Investigation_PrimaryTypes extends BaseClass {

	@Test(priority = -1)

	public void PrimaryTypes() throws InterruptedException, IOException {
		readConfig();
		String path = prop.getProperty("Coppied");
		File file = new File(path);
		FileInputStream fis = new FileInputStream(file);
		XSSFWorkbook wb = new XSSFWorkbook(fis);
		XSSFSheet sheet = wb.getSheet("Investigation_PrimaryTypes");
...
//		System.out.println(sheet.getLastRowNum());
		...
		for (int i = 1; i <= sheet.getLastRowNum(); i++) {
			...
			Thread.sleep(100);
			driver.findElement(By.xpath(prop.getProperty("ptBtn"))).click();
			// press on create PrimaryTypes
